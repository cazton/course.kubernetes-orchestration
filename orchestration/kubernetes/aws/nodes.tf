...
resource "aws_security_group" "eks-nodes" {
  ...
  tags = "${
    map(
     "Name", "${var.nodes_defaults["name"]}-sg",
     "Environment", "${var.env}",
     "kubernetes.io/cluster/${local.cluster_fullname}", "owned"
  )}"
}
...
sed -i s,CLUSTER_NAME,${local.cluster_fullname},g /var/lib/kubelet/kubeconfig
...
resource "aws_autoscaling_group" "autoscaling-group" {
  ...
  tag {
    key                 = "kubernetes.io/cluster/${local.cluster_fullname}"
    value               = "owned"
    propagate_at_launch = true
  }
}
